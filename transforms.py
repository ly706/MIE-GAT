...
class TransverseSection(object):
    def __call__(self, img):
        rate = 0.5
        if random.random() < rate:
...
class RandomRotation(object):
    def __init__(self,angle):
        #angle取90,180,270，其他任意角度也可以，但会造成一定的边界问题
        self.angle=angle
    ...
    def __call__(self,img):
        ...
        if random.random() < rate:
            ...
            for i in range(img.shape[0]):
                ...
                a_new = cv2.warpAffine(A, M, (32, 32))
                a_new = (a_new / 255).astype(np.float32)
                img[i] = a_new
        return img
...
class RandomCrop(object):
    ...
    def __call__(self, img):
        ...
        if self.padding > 0:
            ...
            img1 = np.ones((img.shape[0] + pad, img.shape[1] + pad, img.shape[2] + pad)) # 效果好
            # img1 = np.zeros((img.shape[0] + pad, img.shape[1] + pad, img.shape[2] + pad))
...
class RandomHorizontalFlip(object):
    """HorizontalZeroOutly flip the given PIL.Image randomly with a probability of 0.5."""
